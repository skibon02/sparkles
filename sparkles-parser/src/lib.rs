...
use crate::parsed::{EventName, ParsedEvent, ThreadInfoState};
...
impl SparklesParser {
    ...
    pub fn parse_to_end(&mut self, mut packet_decoder: PacketDecoder, mut f: impl FnMut(&[ParsedEvent], &ThreadInfoState)) -> ParseResult<()> {
        ...
        match packet {
            ...
            Packet::DataBytes(packets) => {
                ...
                for (local_i, (header, data)) in packets.into_iter().enumerate() {
                                        ...
                                        if let Some((start_name, start_ev_type)) = parser_state.id_store.get(start_id) {
                                            ...
                                            if let Some((ev_id, start_tm)) = parser_state.cur_started_ranges.remove(&ord_id) {
                                                ...
                                                let parsed = ParsedEvent::NamedRange {
                                                    name: EventName::new(*start_id, start_name),
                                                    end_name: EventName::new(id, ev_name),
                                                    start: start_tm,
                                                    end: timestamp
                                                };
                                                res.push(parsed);
                                ...
                                if let Some(start_info) = parser_state.cur_started_ranges.remove(&ord_id) {
                                    if let Some((start_name, ev_type)) = parser_state.id_store.get(&start_info.0) {
                                        ...
                                        let parsed = ParsedEvent::Range {
                                            name: EventName::new(start_info.0, start_name),
                                            start: start_info.1,
                                            end: timestamp
                                        };
                                        res.push(parsed);
                    ...
                    f(&res, &parser_state.thread_info_state());
    ...
    pub fn parse_and_convert_to_perfetto(&mut self, packet_decoder: PacketDecoder) -> ParseResult<bytes::BytesMut> {
        ...
        self.parse_to_end(packet_decoder, |evs, thread_info| {
